...
async function parseMealPdf(url: string): Promise<DailyMenu[]> {
  try {
    ...
    console.log("menuLines:", menuLines); // 전체 menuLines 배열 확인
    console.log("menuLines[0]:", menuLines[0]); // 첫 번째 줄 확인
    console.log("menuLines[1]:", menuLines[1]); // 첫 번째 줄 확인

    const dates = menuLines[0]
  .split(/(?=\d{2}월)/)  // 두 자리 숫자 + '월'을 기준으로 분리
  .filter(date => date.includes('월') && date.includes('일'))
  .map(date => date.trim());

console.log("파싱된 날짜:", dates);
...
// 사용 예시
const pdfUrl = "http://www.pvv.co.kr/bbs/download.php?bbsMode=fileDown&code=bbs_menu01&id=741&filename=%C6%C7%B1%B311%BF%F92%C1%D6(1).pdf";
